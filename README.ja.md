...
# 🦁 XXD Panel 064｜インディー・コラージュ頁

### 写真を、静かで不完全な、時間に満ちた古い紙のコラージュ ファイルに変換します
...
古い写真、旅の記録、個人的な思い出に時間の感触を与えたいとき、既成のヴィンテージフィルターでは黄ばみと古びた加工だけが残りがちです。不自然に見えるうえ、何を残すべきなのかも伝わりません。ページいっぱいの切り貼りも、にぎやかなスクラップブックになり、静けさや距離感を失いやすくなります。

**Panel 064** は、最も大切な主体らしさ、関係、感情を、少数の古紙片、淡い線画、文字の断片へ分解します。破れた縁、欠落、わずかなずれ、大きな空白が、信じられる個人資料の感触をつくります。原写真から拾う 1〜2 色は、記憶を呼び覚ますためだけに使います。

### このような場面に適しています

- **写真そのものに記憶や資料としての性格がある：** 家族、旅、人物、古い品、場所を静かに整理し直したい。
- **欠けている感覚は表したいが、見分けやすさは失えない：** 完全な風景と二次的な細部だけを省き、核となる主体らしさと関係は読み取れるようにしたい。
- **時間の感触は欲しいが、ありきたりなヴィンテージフィルターは避けたい：** 強い黄ばみ加工よりも、古紙本来の色、破れた縁、淡い線画、文字の断片で抑制して表現したい。
...
> XXD Panel 064 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
**キーワード:** 古い紙のコラージュ · 手で引き裂かれたエッジ · アーカイブの雰囲気 · タイプライターの小さな活字 · 詩的な空白

Panel 064 は、手で引き裂いた紙の端、古い紙のテクスチャ、鉛筆やインクの線、薄めたテキストの断片、およびごく少量のソース画像のアクセントカラーを使用して、白い空間に空気、時間、静寂、記憶を表現します。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 064 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、静かで不完全な、時間に満ちた古い紙のコラージュ ファイルに変換します |
| **ひと目で分かる特徴** | 古い紙のコラージュ · 手で引き裂かれたエッジ · アーカイブの雰囲気 · タイプライターの小さな活字 · 詩的な空白 |
...
保存する価値のあるアイデンティティ、関係性、感情を抽出し、いくつかの破れた紙片、鉛筆とインクの跡、テキストの残骸からそれらを再構築します。壊れたエッジ、わずかなずれ、広い空き領域が、静かで信頼できるプライベートアーカイブを作成します。

```text
アイデンティティと物語の核をロックする → 完全なシーンと二次的なディテールを削除する → 古くなった紙片、軽い描画、アーカイブの残骸で再構築する → 破れたエッジ、ずれ、不完全さを保存する → ソースからの 1 つまたは 2 つの記憶色で照らす → 空のフィールド内にタイプライターのマイクロコピーを配置する
```

## 完成作品を見分けるポイント

- 古い紙のコラージュ、手でちぎったエッジ、鉛筆、インクが、重厚なビンテージ効果ではなく、繊細なアーカイブの質感を生み出します。
- 主題が完全に再描画されることはありません。認識するには十分な形式と関係だけが残っています。
- 紙、淡い灰色、明るい茶色、古いインクが大半を占めますが、1 つまたは 2 つのソース カラーは記憶マーカーとしてのみ表示されます。
- 被写体が縮小したり、移動したり、浮いたり、切り取られたりするため、何もない空間に時間、沈黙、距離が生まれます。
- 小さなタイプライターのコピーは、水平、垂直、緩く、または端に沿って実行され、コラージュやスペースと組み合わされます。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-064-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 064 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-064 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-064 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-064 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-064 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 064 を適用し、共通設定は一度だけ解決
/xxd-panel-064 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-064`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-064-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-064-prompt.zh-CN.md)
...
| **[xxd-panel-064](https://github.com/nevertoday/xxd-panel-064)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–064） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
