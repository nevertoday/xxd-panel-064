...
That frustration is why I made **Panel 064**. It does not press a template onto your image. It first works out what must not be lost, then translates it into **Turn photos into old paper collage files that are quiet, incomplete and full of time**.
...
It distils the identity, relationships, and emotion worth preserving, then rebuilds them from a few torn-paper pieces, pencil-and-ink marks, and textual remnants. Broken edges, slight displacement, and broad empty areas create a quiet, believable private archive.

```text
lock identity and narrative core → remove the complete scene and secondary detail → rebuild with aged-paper pieces, light drawing, and archival remnants → preserve torn edges, displacement, and incompleteness → illuminate with one or two memory colours from the source → place typewriter microcopy inside the empty field
```

## What makes the finished style recognizable

- Aged-paper collage, hand-torn edges, pencil, and ink create a delicate archival texture rather than a heavy vintage effect.
- The subject is never fully redrawn; only enough form and relationship remain for recognition.
- Paper, pale grey, light brown, and old ink dominate, while one or two source colours appear only as memory markers.
- The subject may shrink, shift, float, or crop so empty space can carry time, silence, and distance.
- Small typewriter copy may run horizontally, vertically, loosely, or along an edge, composing together with collage and space.

Aesthetic constraints and rejection rules live only in the [original source brief](references/original-prompt/zh-CN.md); the Skill and runtime adapter handle delivery variables only. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-064-prompt.en.md)
...
| **[xxd-panel-064](https://github.com/nevertoday/xxd-panel-064)** | BLACK PRIMARY FORM · IMMENSE NEGATIVE SPACE · HALFTONE DISSOLUTION · ZEN REFLECTION · FRAGMENTS OF THOUGHT |
...
| **Soldiers** | `xxd-panel-NNN` (currently 001–064) | Each numbered Skill executes only its own original brief and aesthetic, completing the individual job assigned by the General. |
