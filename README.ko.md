...
# 🦁 XXD Panel 064｜인디 콜라주 페이지

### 사진을 조용하고 불완전하며 시간이 가득한 오래된 종이 콜라주 파일로 바꿔보세요
...
오래된 사진, 여행 기록, 개인적인 기억에 시간의 감각을 더하려고 빈티지 필터를 바로 씌우면 누렇게 바랜 인위적인 효과만 남기 쉽고, 무엇을 간직해야 하는지는 말해 주지 못합니다. 화면을 가득 채운 스크랩북식 콜라주는 산만하고 활기찬 다이어리처럼 되어 고요함과 거리를 잃기 쉽습니다.

**Panel 064**는 가장 중요한 정체성과 관계, 감정을 소수의 낡은 종이 조각, 가벼운 선화, 텍스트 파편으로 나눕니다. 찢긴 가장자리와 누락, 가벼운 어긋남, 넓은 빈 공간이 믿을 만한 개인 기록물의 감각을 만들고, 원본에서 가져온 1–2가지 색은 오직 기억을 불러내는 역할만 합니다.

### 이런 상황에 적합합니다

- **사진 자체에 기억이나 기록의 성격이 있을 때:** 가족, 여행, 인물, 오래된 물건과 장소를 차분하게 다시 정리해야 합니다.
- **불완전함을 표현하되 식별성은 잃을 수 없을 때:** 완전한 장면과 부차적인 디테일만 덜어 내고, 핵심 정체성과 관계는 알아볼 수 있게 남깁니다.
- **시간의 감각은 필요하지만 촌스러운 빈티지 필터는 싫을 때:** 낡은 종이의 본래 색, 찢긴 가장자리, 가벼운 선화와 텍스트 파편이 과도한 황변보다 절제된 분위기를 냅니다.
...
> XXD Panel 064으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
**키워드:** 오래된 종이 콜라주 · 손으로 찢은 가장자리 · 아카이브 느낌 · 타자기 작은 활자체 · 시적인 여백

Panel 064는 손으로 찢은 종이 가장자리, 오래된 종이 질감, 연필 또는 잉크 선, 희석된 텍스트 조각 및 아주 적은 양의 소스 이미지 액센트 색상을 사용하여 공백에 공기, 시간, 침묵 및 기억을 담도록 합니다.

<!-- xxd-panel-benefit:start -->
## 빠른 판단: XXD Panel 064이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 조용하고 불완전하며 시간이 가득한 오래된 종이 콜라주 파일로 바꿔보세요 |
| **한눈에 알아볼 특징** | 오래된 종이 콜라주 · 손으로 찢은 가장자리 · 아카이브 느낌 · 타자기 작은 활자체 · 시적인 여백 |
...
보존할 가치가 있는 정체성, 관계, 감정을 추출한 다음 찢어진 종이 조각, 연필과 잉크 자국, 텍스트 잔재로 이를 재구성합니다. 깨진 모서리, 약간의 이동, 넓은 빈 공간은 조용하고 믿을 수 있는 개인 아카이브를 만듭니다.

```text
정체성과 내러티브 핵심 잠금 → 전체 장면 및 2차 세부 사항 제거 → 오래된 종이 조각, 가벼운 그림 및 보관 잔재로 재구성 → 찢어진 가장자리, 변위 및 불완전성 보존 → 소스에서 하나 또는 두 가지 기억 색상으로 조명 → 빈 필드 안에 타자기 마이크로카피 배치
```

## 완성 스타일을 알아보는 특징

- 오래된 종이 콜라주, 손으로 찢은 가장자리, 연필, 잉크는 무거운 빈티지 효과보다는 섬세한 아카이브 질감을 만들어냅니다.
- 주제는 완전히 다시 그려지지 않습니다. 인식할 수 있을 만큼의 형태와 관계만 남습니다.
- 종이, 옅은 회색, 연한 갈색, 오래된 잉크가 지배적이며 하나 또는 두 가지 원본 색상은 메모리 마커로만 나타납니다.
- 빈 공간이 시간, 침묵, 거리를 전달할 수 있도록 피사체는 줄어들고, 이동하고, 부유하고, 잘릴 수 있습니다.
- 작은 타자기 사본은 수평, 수직, 느슨하게 또는 가장자리를 따라 실행되어 콜라주 및 공간과 함께 구성될 수 있습니다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-064-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 064의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-064 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-064 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-064 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-064 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-064 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-064 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 064 적용, 공통 설정은 한 번만 확정
/xxd-panel-064 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-064`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-064-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-064-prompt.zh-CN.md)
...
| **[xxd-panel-064](https://github.com/nevertoday/xxd-panel-064)** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–064) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
