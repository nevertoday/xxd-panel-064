...
**Panel 064** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**把照片变成安静、残缺而富有时间感的旧纸拼贴档案**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
提炼照片里最值得留下的身份、关系和情绪，把它们拆成少量手撕旧纸、铅笔墨线与文字残片。残缺边缘、轻微错位和大面积空白共同制造一页安静而可信的私人档案。

```text
锁定身份与叙事核心 → 删除完整场景与次要细节 → 拆解为旧纸片、轻线稿和档案残片 → 保留撕边、错位与不完整 → 用 1–2 枚源图记忆色点亮 → 让打字机小字进入留白
```

## 成品中最容易识别的风格特点

- 旧纸拼贴、手撕边缘、铅笔与墨线共同构成细腻而非厚重的档案质感。
- 主体不完整描摹，只保留足以辨认的少量形态与关系。
- 纸张本色、浅灰、淡褐和旧墨为主，1–2 种源图颜色只作局部记忆标记。
- 主体可缩小、偏置、悬置或裁切，让空白承担时间、沉默与距离。
- 小型打字机文字可横排、竖排、散落或贴边，与拼贴和留白共同构图。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-064-prompt.en.md)
...
| **[xxd-panel-064](https://github.com/nevertoday/xxd-panel-064)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–064） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
